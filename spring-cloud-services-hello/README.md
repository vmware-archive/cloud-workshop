...
This repository contains samples apps which can be deployed to a Pivotal Cloud Foundry environment installed with Spring Cloud Services. There are two applications, hello-service and hello-client. Build and run each of these in order and note the differences between each version as you go though them, so you can see what needs to be added to an application to use each of the Spring Cloud Services.
